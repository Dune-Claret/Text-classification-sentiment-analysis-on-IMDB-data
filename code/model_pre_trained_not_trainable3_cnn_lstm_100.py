...
from keras.layers import LSTM, Embedding, Flatten, Dense, TimeDistributed, Reshape, RepeatVector
...
# Model with an embedding layer not trainable & 1 LSTM & 1 CNN & 10 epochs
K.clear_session()
#for k in range(10) :
...
filepath="model_not_train3_lstm_100_cnn_weights-improvement-{epoch:02d}-{val_acc:.2f}.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True, mode='max')
callbacks_list = [checkpoint]

model_glove_not_train_3_lstm_100_cnn_1_history = model_glove.fit(x_train, y_train, epochs=10, batch_size=32, validation_data=(x_val, y_val), verbose = 2, callbacks = callbacks_list)
    
    # with open('model_glove_not_train_3_lstm_100_cnn_1_history{}.pkl'.format(k),'wb') as f:
    #     pickle.dump(model_glove_not_train_3_lstm_100_cnn_1_history.history, f)
...
model_not_trainable3_cnn_lstm_100.load_weights("model_not_train3_lstm_100_cnn_weights-improvement-04-0.82.hdf5")
model_not_trainable3_cnn_lstm_100.compile(optimizer='adam', loss='binary_crossentropy', metrics=['acc'])

print(model_not_trainable3_cnn_lstm_100.evaluate(x_test, y_test, verbose = 1))