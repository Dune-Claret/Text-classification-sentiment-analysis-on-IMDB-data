...
filepath="model_not_train3_bilstm_50_weights-improvement-{epoch:02d}-{val_acc:.2f}.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True, mode='max')
callbacks_list = [checkpoint]

model_glove_not_train_3_bilstm_50_history = model_glove.fit(x_train, y_train, epochs=10, batch_size=32, validation_data=(x_val, y_val), verbose = 2, callbacks = callbacks_list)

    # with open('model_glove_not_train_3_bilstm_50_history{}.pkl'.format(k),'wb') as f:
    #     pickle.dump(model_glove_not_train_3_bilstm_50_history.history, f)
...
model_not_trainable3_bilstm_50.load_weights("model_not_train3_bilstm_50_weights-improvement-09-0.85.hdf5")
model_not_trainable3_bilstm_50.compile(optimizer='adam', loss='binary_crossentropy', metrics=['acc'])

print(model_not_trainable3_bilstm_50.evaluate(x_test, y_test, verbose = 1))