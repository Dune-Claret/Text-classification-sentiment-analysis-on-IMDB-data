...
filepath="model_train3_weights-improvement-{epoch:02d}-{val_acc:.2f}.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True, mode='max')
callbacks_list = [checkpoint]

model_glove_train_3_history = model_glove.fit(x_train, y_train, epochs=10, batch_size=32, validation_data=(x_val, y_val), verbose = 2, callbacks = callbacks_list)

# with open('model_glove_train_3_history{}.pkl'.format(k), 'wb') as f:
#     pickle.dump(model_glove_train_3_history.history, f)
...
model_trainable3.load_weights("model_train3_weights-improvement-02-0.83.hdf5")
model_trainable3.compile(optimizer='adam', loss='binary_crossentropy', metrics=['acc'])

print(model_trainable3.evaluate(x_test, y_test, verbose = 1))