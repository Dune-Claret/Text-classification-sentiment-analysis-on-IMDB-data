...
filepath="model_not_train3_lstm_50_attention_weights-improvement-{epoch:02d}-{val_acc:.2f}.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True, mode='max')
callbacks_list = [checkpoint]

    
model_glove_not_train_3_no_cnn_attention_history = model_glove.fit(x_train, y_train, epochs=10, batch_size=32, validation_data=(x_val, y_val), verbose = 2, callbacks = callbacks_list)
            
#    with open('model_glove_not_train_3_no_cnn_attention_history{}.pkl'.format(k), 'wb') as f:
#        pickle.dump(model_glove_not_train_3_no_cnn_attention_history.history, f)
...
model_not_trainable3_lstm_50_cnn.load_weights("model_not_train3_lstm_50_attention_weights-improvement-10-0.85.hdf5")
model_not_trainable3_lstm_50_cnn.compile(optimizer='adam', loss='binary_crossentropy', metrics=['acc'])

print(model_not_trainable3_lstm_50_cnn.evaluate(x_test, y_test, verbose = 1))