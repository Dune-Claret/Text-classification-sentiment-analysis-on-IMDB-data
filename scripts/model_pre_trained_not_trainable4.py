...
# Model with an embedding layer not trainable & 5 dense layers (4 hidden) & 10 epochs
K.clear_session()
# for k in range (10) :
# print(k)
...
model_glove.add(Dense(4, activation='relu'))
model_glove.add(Dense(1, activation='sigmoid'))
model_glove.summary()

model_glove.compile(optimizer='adam', loss='binary_crossentropy', metrics=['acc'])
filepath="model_not_train4_weights-improvement-{epoch:02d}-{val_acc:.2f}.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True, mode='max')
callbacks_list = [checkpoint]

model_glove_not_train_4_history = model_glove.fit(x_train, y_train, epochs=10, batch_size=32, validation_data=(x_val, y_val), verbose = 2, callbacks = callbacks_list)

# with open('model_glove_not_train_4_history{}.pkl'.format(k),'wb') as f:
#     pickle.dump(model_glove_not_train_4_history.history, f)
...
model_not_trainable4.load_weights("model_not_train4_weights-improvement-10-0.50.hdf5")
model_not_trainable4.compile(optimizer='adam', loss='binary_crossentropy', metrics=['acc'])

print(model_not_trainable4.evaluate(x_test, y_test))